...
@Slf4j
@Singleton
class MKHideTannerOverlay extends OverlayPanel
{
	private final Client client;
	private final MKHideTannerPlugin plugin;
	private final MKHideTannerConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private MKHideTannerOverlay(final Client client, final MKHideTannerPlugin plugin, final MKHideTannerConfig config)
	{
		super(plugin);
		...
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "Hide Tanner overlay"));
	}
	...
	public Dimension render(Graphics2D graphics)
	{
		...
		tableDelayComponent.addRow("Sleep delay:", MKHideTannerPlugin.sleepLength + "ms");
